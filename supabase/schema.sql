...
-- ------------------------------------------------------------
-- INVENTARIO DE INSUMOS (parte del módulo Costos y Recetas)
-- Telas, hilos, botones, empaque, mano de obra: NO son productos de
-- venta, por eso viven aparte de `productos` (cuyo catálogo completo
-- alimenta los selectores de Ventas). Ver migración 027.
-- Declarados aquí porque referencian `proveedores`.
-- ------------------------------------------------------------
create table nativo.insumos (
  id bigint generated always as identity primary key,
  nombre text not null unique,
  codigo text,
  categoria text,
...
create unique index idx_insumos_codigo on nativo.insumos (codigo) where codigo is not null;
create index idx_insumos_categoria on nativo.insumos (categoria);
create index idx_insumos_activo on nativo.insumos (activo);

-- Movimientos de insumos. Nunca se borran (mismo criterio que el kardex de
-- productos): FK blanda + nombre copiado para sobrevivir a la eliminación.
create table nativo.insumos_movimientos (
  id bigint generated always as identity primary key,
  fecha timestamptz not null default now(),
  tipo text not null check (tipo in ('entrada', 'salida', 'ajuste')),
  insumo_id bigint references nativo.insumos (id) on delete set null,
  insumo text not null,
  cantidad numeric not null,           -- siempre con signo; en 'ajuste' es la diferencia
  costo_unitario numeric,
  costo_total numeric,
...
create index idx_insumos_mov_insumo on nativo.insumos_movimientos (insumo_id, fecha desc);
create index idx_insumos_mov_tipo on nativo.insumos_movimientos (tipo);
create index idx_insumos_mov_fecha on nativo.insumos_movimientos (fecha desc);

-- Las líneas de receta pueden apuntar a un insumo (además de poder apuntar a
-- un producto del catálogo, cuando el material sí se compra terminado).
-- Se agrega aquí porque `recetas_materiales` se declara antes que `insumos`.
alter table nativo.recetas_materiales
  add column insumo_id bigint references nativo.insumos (id) on delete set null;
create index idx_recetas_materiales_insumo on nativo.recetas_materiales (insumo_id);
...
-- ------------------------------------------------------------
-- FUNCIONES RPC: borrado de movimientos financieros (migración 028).
-- Atómicas porque tocan varias tablas: el asiento bancario debe morir
-- junto con el pago o el saldo de la cuenta queda mal. Cuidado con
-- `movimientos_bancarios.pago_gasto_id/pago_ingreso_id`, que son
-- `on delete set null`: si se borra el pago primero, el asiento queda
-- huérfano y sigue sumando en la cuenta.
-- ------------------------------------------------------------
...
-- Se BLOQUEA si el gasto nació de recibir una orden de compra: ese gasto es la
-- cuenta por pagar de mercancía que YA entró al inventario.
...
-- Solo movimientos manuales y transferencias. Los que nacen de un pago se
-- deshacen anulando el pago, para que el gasto/ingreso/venta se recalcule.
...
-- ------------------------------------------------------------
-- FUNCIÓN RPC: movimiento de insumo atómico.
-- Existencia + costo promedio ponderado + asiento de movimiento.
-- Ver migración 027.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- Semillas: categorías de insumo (inventario de insumos)
-- ------------------------------------------------------------
insert into nativo.listas_maestras (tipo, valor) values
